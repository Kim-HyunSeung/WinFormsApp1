...
namespace MainForms
{
    public partial class M03_Main : Form
    {
        ...
        private void btnFunction_Click(object sender, EventArgs e)
        {
            ...
            //if (tsFunction.Name == "btnSearch") DoFunction("SEARCH");
            //else if (tsFunction.Name == "btnAdd") DoFunction("ADD");
            //else if (tsFunction.Name == "btnDelete") DoFunction("DELETE");
            //else if (tsFunction.Name == "btnSave") DoFunction("SAVE");
        }

        void DoFunction(string states)
        {
            // 오픈되어있는 페이지가 없을 경우 리턴
            if (myTabControl1.TabPages.Count == 0) return;
            ...
            // 캐스팅 : 상속받은 부모 클래스로 부터 형 변환이 가능 할 경우 형변호나을
            //          명시적으로 실행하는 기능.

            // as : 대상으로부터 상속받은 클래스 이면, 형변환을 수행하고,
            //      그렇지 않으면 null 값을 대입하는 연산자

            // is : 대상으로부터 상속 받았는지 여부를 bool 형식으로 결과값만 변환.



            //myTabControl1.SelectedTab.Controls[0] : page 에 추가된 컨트롤 중에 최상위 컨트롤
            //                                       즉 Form04_ItemMasetr
            //처음에 상속을 받았는지 안받았는지 확인.
           
            if (myTabControl1.SelectedTab.Controls[0] is BaseChildForm == false) return;       // 1번
            //////is BaseChildForm : 앞에 내용이 요기서 상속받았다면 이라는뜻.
            ...
            // 상속이 된다면 바로 형변환.
            // BaseChildForm Child = myTabControl1.SelectedTab.Controls[0] as BaseChildForm;    // 2번
            // if (Child == null) return;


            #endregion

            if (states == "조회")  Child.DoInquire();
            else if (states == "추가") Child.DoNew();
            else if (states == "삭제") Child.DoDelete();
            else if (states == "저장") Child.DoSave();
        }
        #endregion
