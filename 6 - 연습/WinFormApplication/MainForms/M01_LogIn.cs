...
// 공통 로직 및 변수 등을 관리하는 우리가 만든 클래스 라이브러리 (API)
using Assamble;
        ...
        private void DoLogin()
        {
            ...
            try
            {
               ...
               // 로그인 할수 있는 id 와 비밀번호 입력했는지 확인.
                ...
                #region <비밀번호 실패 횟수를 DB에 저장하고 프로그램이 종료 된 후에도 종료된후 다시 실행 시켜도 로그인이 되지 않도록 설정.> 
                ...
                //  sSelectSQL       += $"   AND PW     = '{sPassWord}'     ";
                //  sSelectSQL       += $"   AND PWK    = {PWCnt}           ";
                ...
                //  sCmd.CommandText = sSelectSQL;

                //  sCmd.ExecuteNonQuery();
                //  MessageBox.Show($"{PWCnt} 회 초과하셨습니다. 관리자에게 문의하세요.");
                //  sTran.Commit();
                ...
                this.Tag = true;  // 모든 데이터값들을 담을수 있는 Tag임.
                this.Close();
            }
            catch (Exception ex)
            {
